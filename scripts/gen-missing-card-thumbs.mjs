...
const WIDTH = 800, HEIGHT = 520, QUALITY = 88; // 2x retina-safe for the 400x260 card display size (see recent-card-img in index.astro)
...
for (const name of files) {
  ...
  const jpgOut = join(CARD_DIR, `${base}.jpg`);
  const webpOut = join(CARD_DIR, `${base}.webp`);
  ...
  console.log(`${name} -> card/${base}.jpg + .webp  (${(jpgBuf.length/1024).toFixed(0)}KB, ${(webpBuf.length/1024).toFixed(0)}KB)`);
}
console.log('done');
