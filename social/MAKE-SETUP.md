...
1. Open the CSV: `social/queue-sheet.csv` (rows for UK — order, slug,
   image_url, caption, posted).
2. New Google Sheet → **File → Import → Upload → `queue-sheet.csv` →
   "Replace spreadsheet"**. Keep the header row.
3. Name it `UK Instagram queue`. Leave the `posted` column blank.
...
| Field | Value |
|---|---|
| Spreadsheet | `UK Instagram queue` |
| Sheet | `Sheet1` |
