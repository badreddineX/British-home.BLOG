...
const IMG_BASE = 'https://raw.githubusercontent.com/badreddineX/British-home.BLOG/main/social-posts/ig-2026-09/';
const BASE_TAGS = ['#ukhomedecor', '#britishhome'];
const AUDIENCE = 'UK homes and renters';
// -------------------------
...
for (const f of readdirSync(BLOG).filter((x) => x.endsWith('.md'))) {
  ...
  items.push({ slug, title, link: `https://britishhomeinterior.co.uk/blog/${slug}?utm_source=facebook&utm_medium=social&utm_campaign=uk_fb`, fbText: desc, date: scalar(fm, 'datePublished'), image: `${IMG_REL}/${img}`, imageUrl: IMG_BASE + img, igCaption: caption });
}
