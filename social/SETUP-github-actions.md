...
2. Use case **Other** → type **Business** → name it e.g. `britishhomeinterior-social`.
...
- When the queue gets low: add a caption block to
  `social-posts/CAPTIONS-UK.md`, drop the rendered image in
  `social-posts/ready-UK/NN-<slug>.jpg`, run `node social/build-queue.mjs`,
  commit.
