...
export default async function handler(req, res) {
  ...
  try {
    ...
    return statusPage(res, 200, 'You’re unsubscribed',
      'You won’t receive any more emails from British Home Interior. Changed your mind? You can subscribe again anytime.');
  } catch (err) {
    console.error('[newsletter] unsubscribe failed:', err);
    if (isPost) { res.statusCode = 500; return res.end(); }
    return statusPage(res, 500, 'Something went wrong',
      'We could not process that right now. Email hello@britishhomeinterior.co.uk and we’ll remove you by hand.');
  }
}
