...
const SITE_URL = (process.env.SITE_URL || 'https://britishhomeinterior.co.uk').replace(/\/$/, '');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const feedRes = await fetch(`${SITE_URL}/newsletter-feed.json`, {
      headers: { 'User-Agent': 'britishhomeinterior-broadcast' },
    });
    ...
    // actually went out — a fully failed send (e.g. unverified domain) leaves
    // everything queued so the next run retries it.
