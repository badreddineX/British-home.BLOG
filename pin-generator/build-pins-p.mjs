...
const PRIMARY = {};
const ROOM_KW = [
  [/living|lounge|sofa|scandi|maximalist|colour-schemes/i, ['small living room ideas uk', 'very small living room ideas', 'modern small living room ideas', 'small living room ideas with tv', 'living room decor ideas']],
...
function tmpl(p, layout) {
  ...
  if (layout === 2) { // split: photo top, solid panel bottom
    return `<style>${BASE} body{background:#2A3325}
      ...
      <div class="top"></div><div class="panel"><div><span class="pill">${esc(p.label)}</span><div class="h" style="margin-top:34px">${esc(p.headline)}</div></div><div class="dom" style="color:#C8A24A">${DOMAIN}</div></div>`;
  }
  // 1 = overlay, 3 = overlay + number badge
  const badge = layout === 3 && p.number
    ? `<div style="position:absolute;top:56px;left:56px;width:230px;height:230px;border-radius:50%;background:#C8A24A;color:#1B211D;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 8px 30px rgba(0,0,0,.35)"><div style="font-weight:800;font-size:120px;line-height:1">${p.number}</div><div style="font-weight:800;font-size:26px;letter-spacing:.18em">IDEAS</div></div>` : '';
